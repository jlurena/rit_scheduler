...
public abstract class BoomButton extends FrameLayout {
    ...
    // Inner View
    protected ViewGroup innerView;
    protected @LayoutRes int innerViewRes;
    ...
    protected void initAttrs(BoomButtonBuilder builder) {
        ...
        innerView = builder.innerView;
        innerViewRes = builder.innerViewRes;
        innerViewRect = builder.viewRect;

        if (buttonEnum == ButtonEnum.SimpleCircle || buttonEnum == ButtonEnum.TextInsideCircle || buttonEnum == ButtonEnum.TextOutsideCircle) {
            if (isRound)
                buttonCornerRadius = builder.buttonRadius;
            else
    ...
    protected void initSubText(ViewGroup parent) {
        ...
        if (subTypeface != null)
            subText.setTypeface(subTypeface);
    ...
    protected void initCircleButtonDrawable() {
        if (rippleEffectWorks) {
            GradientDrawable gradientDrawable = isRound ? Util.getOvalDrawable(button, unable ? unableColor() : normalColor()) : Util.getRectangleDrawable(button, buttonCornerRadius, unable ? unableColor() : normalColor());
            RippleDrawable rippleDrawable = new RippleDrawable(ColorStateList.valueOf(highlightedColor()), gradientDrawable, null);
        ...
        button.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (!touchable)
                    return;
                if (listener != null)
                    listener.onButtonClick(index, BoomButton.this);
                if (onBMClickListener != null)
    ...
    void updateImageRect() {
        LayoutParams params = new LayoutParams(imageRect.right - imageRect.left, imageRect.bottom - imageRect.top);
    ...
    void updateImagePadding() {
        if (imagePadding != null && image != null) {
            image.setPadding(imagePadding.left, imagePadding.top, imagePadding.right, imagePadding.bottom);
        }
    }

    void updateTextRect() {
        FrameLayout.LayoutParams params = new LayoutParams(textRect.right - textRect.left, textRect.bottom - textRect.top);
    ...
    void updateTextPadding() {
        if (textPadding != null && text != null) {
            text.setPadding(textPadding.left, textPadding.top, textPadding.right, textPadding.bottom);
        }
    }
    ...
    void updateSubTextRect() {
        FrameLayout.LayoutParams params = new LayoutParams(subTextRect.right - subTextRect.left, subTextRect.bottom - subTextRect.top);
    ...
    void updateSubTextPadding() {
        if (subTextPadding != null && subText != null) {
            subText.setPadding(subTextPadding.left, subTextPadding.top, subTextPadding.right, subTextPadding.bottom);
        }
    }
    ...
    protected void toNormalImage() {
        if (unable)
            Util.setDrawable(image, unableImageRes, unableImageDrawable);
        else
            Util.setDrawable(image, normalImageRes, normalImageDrawable);
    }
    ...
    protected void toHighlightedImage() {
        if (unable)
            ...
            Util.setDrawable(image, highlightedImageRes, highlightedImageDrawable);
    }
    ...
    public int pieceColor() {
        if (pieceColor == null && pieceColorRes == 0)
            ...
            return Util.getColor(context, pieceColorRes);
        else
    ...
    public boolean isNeededColorAnimation() {
        ...
        if (unable)
            return pieceColor.compareTo(unableColor()) != 0;
        else
            return pieceColor.compareTo(normalColor()) != 0;
    }
    ...
    public ViewGroup setInnerView(@LayoutRes int layoutRes) {
        if (layoutRes != 0) {
            innerView = (ViewGroup) LayoutInflater.from(context).inflate(layoutRes, button, false);
            updateInnerViewRect();
            button.addView(innerView);
        }
        return innerView;
    }
}
